<div id="nav-bar">
    <div id="nav-bar-logo">
        <img src="img/logo_tirpitz_transparente.png" alt="Imagen tirpitz">
    </div>
    <ul id="nav-bar__ul">
        <li class="nav-bar__li"><a href="home.php">Inicio</a></li>
        <li class="nav-bar__li"><a href="estudiantes.php">Estudiantes</a></li>
        <li class="nav-bar__li"><a href="grupos.php">Grupos</a></li>
        <div class="div-with-subnav">
            <li class="nav-bar__li"><a href="#">Servicios</a><i class="fas fa-angle-down"></i></li>
            <ul class="submenus-container">
                <li class="nav-bar__li "><a class="" href="sistemaPae.php">Sistema PAE</a></li>
                <li class="nav-bar__li disabled"><a class="disabled" href="#">Sistema de asistencia</a></li>
            </ul>
        </div>
        <li class="nav-bar__li"><a href="includes/logout.php">Cerrar Sesion</a></li>
    </ul>
</div>